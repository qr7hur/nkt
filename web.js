...
.get('/PluginManager.js', function(req, res){
	...
	query.on('end', function() {
		client.end();
		fs.readFile(path.resolve(__dirname,'files','PluginManager.js'), 'utf8', function(err, data){
			var newFile=data;
			if(!err){
				for(var i in names){
					if(data.indexOf('addInList('+names[i])<0){
						var addInList="addInList('"+names[i]+"', false);";
						newFile = newFile.replace("$.chat.subscribe(onEvent);",addInList+"\n$.chat.subscribe(onEvent);");
						fs.writeFile(path.resolve(__dirname,'files','plugins','injected',names[i]+'.js'), files[i], function(err){});
					}
				}
				res.send(newFile);
...
.post('/plugin-add', function(req, res) {
	...
	if(req.body.pluginFile.length < 100000){
		...
		client.connect();
		query = client.query(new pg.Query('DELETE FROM plugins WHERE name = $1', [name]));
		query.on('end', function() {
			query = client.query(new pg.Query('INSERT INTO plugins(name,file) VALUES($1,$2)', [name, req.body.pluginFile]));
			query.on('end', function() {
				query = client.query(new pg.Query('DELETE FROM plugins_backup WHERE name = $1 AND file = $2', [name, req.body.pluginFile]));
				query.on('end', function() {
					query = client.query(new pg.Query('INSERT INTO plugins_backup(name,file) VALUES($1,$2)', [name, req.body.pluginFile]));
					query.on('end', function() {
						client.end();
						res.send(200);
					});
...
.post('/plugin-remove', function(req, res) {
	client = new pg.Client(connectionString);
	client.connect();
	query = client.query(new pg.Query('DELETE FROM plugins WHERE name = $1', [req.body.pluginName]));
	query.on('end', function() {
		client.end();
		res.send(200);
	});
...
.post('/plugin-rollback', function(req, res) {
	...
	client = new pg.Client(connectionString);
	client.connect();
	query = client.query(new pg.Query('SELECT * FROM plugins_backup WHERE name = $1', [req.body.pluginName]));
	query.on('row', function(row) {
		names.push(row.name);
		files.push(row.file);
	});
	query.on('end', function(){
		...
		if(randName && randFile){
			query = client.query(new pg.Query('DELETE FROM plugins WHERE name = $1', [randName]));
			query.on('end', function() {
				query = client.query(new pg.Query('INSERT INTO plugins(name,file) VALUES($1,$2)', [randName, randFile]));
				query.on('end', function() {
					client.end();
					res.send(200);
				});
