    ...
    function PluginManager() {
        ...
        _self.loadPlugin = function (name) {
            ...
            if (_active.indexOf(name) < 0) {
                ...
                //                var worker = new Worker( getPath( name, pluginTag.data('official') ) );  
                //                _plugins[name] = worker;
                //                worker.postMessage({
                //                    type: 'init',
                //                    pluginManager: _self
                //                });
        ...
        _self.pluginList = function () {
            ...
            _self.pluginTag.find('#plugin-container').children().each(function () {
        ...
        var addInList = function (name, official) {
            var scrapAlreadyAddedPluginNames = () => {
                var pluginNodes = document.getElementById("plugin-container").childNodes;
                var pluginNames = [];
                for (var i = 0; i < pluginNodes.length; i++) {
                    pluginNames.push(pluginNodes[i].id);
                }
                return pluginNames
            }
